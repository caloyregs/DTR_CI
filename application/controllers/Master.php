...
class Master extends CI_Controller
{
  ...
  public function qr_users($username)
  {
    $d['title'] = 'Users';
    $d['users'] = $this->db->get_where('users', ['username' => $username])->row_array();
    $d['account'] = $this->Admin_model->getAdmin($this->session->userdata['username']);
    ...
    if ($this->form_validation->run() == false) {
      ...
      $this->load->view('templates/sidebar');
      $this->load->view('templates/topbar');
      $this->load->view('master/users/qr_users', $d);
      $this->load->view('templates/footer');
    } else {
      $data = ['qrcode' => $this->input->post('u_username')];
      $this->_editQRUsers($data, $username);
    }
  }
  ...
  private function _editQRUsers($data, $username)
  {
    $this->db->update('users', $data, ['qrcode' => $username]);
    $rows = $this->db->affected_rows();
    if ($rows > 0) {
      $this->session->set_flashdata('message', '<div class="alert alert-success" role="alert">
          Successfully edited an account!</div>');
  ...
  public function post_qr_users() {

      $userName = $this->input->post('username');
      $status = array("STATUS"=>"true");

      $data = ['qrcode' => $userName.'.png'];
      $this->db->update('users', $data, ['username' => $userName]);
      $rows = $this->db->affected_rows();
